...
vars :: Exp -> [IndexedVar]
vars (X v) = [v]
vars (Lam v e) = if elem v (vars e) then vars e else [v] ++ (vars e)
vars (App e1 e2) = union (vars e1) (vars e2)
...
freeVars (X v) = [v]
freeVars (Lam v e) = filter (\x -> x /= v) (vars e)
...
occursFree v e = elem v (freeVars e)
